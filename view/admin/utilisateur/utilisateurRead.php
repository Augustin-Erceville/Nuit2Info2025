    ...
    <div class="card bg-black text-light border border-light">
        <div class="card-header d-flex justify-content-between align-items-center">
            <span class="fw-bold text-uppercase">
                <i class="bi bi-person-badge me-2"></i>Détails de l'utilisateur #<?= htmlspecialchars($utilisateur['id']) ?>
            </span>
            <span class="badge bg-info text-dark"><?= htmlspecialchars($utilisateur['role'] ?? 'eleve') ?></span>
        </div>
        <div class="card-body">
            <dl class="row mb-0">
                <dt class="col-sm-3">Prénom</dt>
                <dd class="col-sm-9"><?= htmlspecialchars($utilisateur['prenom']) ?></dd>

                <dt class="col-sm-3">Nom</dt>
                <dd class="col-sm-9"><?= htmlspecialchars($utilisateur['nom']) ?></dd>

                <dt class="col-sm-3">Email</dt>
                <dd class="col-sm-9"><?= htmlspecialchars($utilisateur['email']) ?></dd>

                <dt class="col-sm-3">Adresse</dt>
                <dd class="col-sm-9"><?= htmlspecialchars($utilisateur['adresse']) ?></dd>

                <dt class="col-sm-3">Téléphone</dt>
                <dd class="col-sm-9"><?= htmlspecialchars($utilisateur['téléphone'] ?? '') ?></dd>

                <dt class="col-sm-3">Date de naissance</dt>
                <dd class="col-sm-9"><?= htmlspecialchars($utilisateur['date_naissance'] ?? '') ?></dd>
            </dl>
        </div>
        <div class="card-footer d-flex justify-content-between">
            ...
            <div>
                <a href="utilisateurUpdate.php?id=<?= urlencode($utilisateur['id']) ?>"
                   class="btn btn-warning me-2">
                    <i class="bi bi-pencil"></i> Modifier
                </a>
                <a href="utilisateurDelete.php?id=<?= urlencode($utilisateur['id']) ?>"
